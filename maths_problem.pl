		...
		foreach my $c (1..9) {
			foreach my $d (1..9) {
				if ( numbers_ok($a, $b, $c, $d)) {
					...
					#print "($a, $b), ($c, $d): $ab - $cd = $ef ($e, $f)\n";
					if (numbers_ok($a, $b, $c, $d, $e, $f)) {
						foreach my $g (1..9) {
							foreach my $h (1..9) {
...
sub max {
	# sub to return the maximum value form a list
	my $max = shift;
	while (my $num = shift) {
		if ($num>$max) { $max = $num;}
	}
...
sub numbers_ok {
	...
	foreach my $element (@_) {
		$count{$element}++;
		print "$element ($count{$element}) " if $debug;
		if ($count{$element}>1) {
			$result = 0;
		}
	}
...
#sub check_legal_solution {
#	# sub to check a solution is legal given $p, @numbers
#	my $p = shift;
#	my @numbers = $_;
#	my $ef = 0;
#	return 0;
